      ...
      <q-toolbar class="text-black">
        ...
        <q-toolbar-title>
          ...
          <div class="text-body2 row items-center">
            <div
              class="q-px-sm cursor-pointer non-selectable relative-position flex flex-center toolbar-a"
              v-ripple
            >
              文件
              <q-menu>
              ...
              <q-menu>
                <q-list dense style="min-width: 100px">
                  ...
                  <q-item clickable @click="zoomIn">
                    ...
                    <q-item-section side> Ctrl+滚轮向上 </q-item-section>
                  </q-item>
                  <q-item clickable @click="zoomOut">
                    <q-item-section> 缩小 </q-item-section>
                    <q-item-section side> Ctrl+滚轮向下 </q-item-section>
                  </q-item>
                  <q-separator />
        ...
        <q-btn
          ...
          dense
          icon="menu"
          v-if="editorStore.showRightDrawer"
          @click="shRightDrawer"
          class="q-ml-md"
        >
...
export default defineComponent({
  ...
  setup() {
    ...
    const refresh = () => {
      _this.parent.parent.proxy.refresh();
    };
    const cancel = () => {
      file.value = null;
    };
    const saveData = () => {
      _this.parent.parent.proxy.saveData();
    };
    const importJson = () => {
      let reader = new FileReader();
      reader.readAsText(file.value);
      reader.onload = () => {
        _this.parent.parent.proxy.importJson(JSON.parse(reader.result));
        file.value = null;
      };
    };
    const shLeftDrawer = () => {
